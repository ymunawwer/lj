...
class database {
    ...
    isFirstUse(){
     ...
     setloanrecord(bookid, amount, date, duedate, give, take, attachment, remarks, partner_contact, phoneid, typedb, mode, installment, totalMonths, interest, loanName, installmentAmount) {
        ...
        db.transaction(tx => {
            tx.executeSql(
                "INSERT INTO loanrecords ( book_id , amount , date , duedate , give , take , attachment , remarks , partner_contact , uploaded , interest , type , mode ,  installment , totalMonths, loanName, installmentAmount ) VALUES (" + bookid + "," + amount + ",'" + date + "', '" + duedate + "', " + give + ", " + take + ", '" + attachment + "', '" + remarks + "', '" + partner_contact + "' ,0, '" + interest + "', '"+typedb+"','"+mode+"','"+installment+"',"+totalMonths+",'"+loanName+"', "+installmentAmount+")",
                [],
                (tx, results) => {
    ...
    setrecord(bookid, amount, duedate, give, take, attachment, remarks, partner_contact, phoneid, type) {
        const date = new Date();
        const uploaded = 0;
        console.log()
        db.transaction(tx => {
            tx.executeSql(
                "INSERT INTO records (book_id, amount, date, duedate, give, take, attachment, remarks, partner_contact, uploaded, phoneid, type) VALUES (" + bookid + "," + amount + ",'" + date + "', '" + duedate + "', " + give + ", " + take + ", '" + attachment + "', '" + remarks + "', '" + partner_contact + "' ,0,"+phoneid+",'"+type+"')",
                [],
                (tx, results) => {
    ...
    getLoanNames(bookid){
        return new Promise((resolve, reject) => {
            ...
            db.transaction(tx => {
                tx.executeSql(
                    "SELECT * FROM loanname WHERE bookid = "+bookid+" ORDER BY lastupdated DESC",
                    [],
                    (tx, results) => {
                        //console.log('get Record Results new ', results.rows['_array']);
                        resolve(results.rows['_array'])
    ...
    getLoanRecordById(id){
        return new Promise((resolve, reject) => {
            ...
            db.transaction(tx => {
                tx.executeSql(
                    "SELECT * FROM loanrecords WHERE recordid=" + id,
                    [],
                    (tx, results) => {
                        console.log('get Record Results new ', results.rows['_array']);
                        resolve(results.rows['_array'])
    ...
    getLoanRecord(bookid) {
        return new Promise((resolve, reject) => {
            ...
            db.transaction(tx => {
                tx.executeSql(
                    "SELECT * FROM loanrecords WHERE book_id=" + bookid,
                    [],
                    (tx, results) => {
                        console.log('get Record Results new ', results.rows['_array']);
                        resolve(results.rows['_array'])
