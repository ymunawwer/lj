...
export default function EntryDetails(props) {
    ...
    console.log("store", storeObject.getRecordLoanYes())
        ...
        (async () => {
            ...
            try {
                if(storeObject.getRecordLoanYes()===0){
                    ...
                    setRecords(records)
                    console.log("records attachment", records[0].attachment)
                     setIsLoan(0)
                }else{
        ...
        <View style={{flex:1}}>
            ...
            {/* One Contact */}
                        ...
                        
                    {
                        ...
                        console.log(''):
                        mRecords?mRecords[0].take===0?
                        <View style={[styles.row,{borderRadius:0}]}><Text>Running Balance</Text><Text style={styles.giveAmountText}>₹{(getTotalAmount(mRecords[0].amount, mRecords[0].interest, mRecords[0].installment, mRecords[0].totalMonths) + mRecords[0].amount)}</Text></View>
                        :<View style={[styles.row,{borderRadius:0}]}><Text>Running Balance</Text><Text style={styles.takeAmountText}>₹{(getTotalAmount(mRecords[0].amount, mRecords[0].interest, mRecords[0].installment, mRecords[0].totalMonths) + mRecords[0].amount)}</Text></View>:
                        console.log('')
                    }
            ...
            <View style={[styles.container]}>
                    ...
                    <View style={{marginBottom:10}}>
                        ...
                        <View>
                            <Text style={[stylesI.greyTextSm]}>{mRecords?"You gave ₹"+mRecords[0].amount+" to "+mRecords[0].partner_contact:console.log('')}</Text>
                                    ...
                                    <View >
                                        {
                                            mRecords[0].give === 1 ?
                                                <Text
                                                    style={[styles.greyTextSm, styles.giveAmountText]}>{'Principal ₹ ' + mRecords[0].amount}</Text> :
                                                <Text
                                                    style={[styles.greyTextSm, styles.takeAmountText]}>{'Pricipal ₹ ' + mRecords[0].amount}</Text>
                                        }
                                        <Text
                                            style={[styles.greyTextSm, styles.boldText, {color: "#f9c032"}]}>{' Interest         + ₹ ' + getTotalAmount(mRecords[0].amount, mRecords[0].interest, mRecords[0].installment, mRecords[0].totalMonths)}</Text>
                                        <Text
                                            style={[styles.greyTextSm, styles.boldText, {color: "black"}]}>{' Final              = ₹ ' + (getTotalAmount(mRecords[0].amount, mRecords[0].interest, mRecords[0].installment, mRecords[0].totalMonths) + mRecords[0].amount)}</Text>
